// ============================================================
// ЛОГИКА ЭСКАЛАЦИИ С ПОДДЕРЖКОЙ ЕЖЕНЕДЕЛЬНОГО ВАРИАТОРА
// ============================================================

// ============================================================
// ЕЖЕНЕДЕЛЬНЫЙ ВАРИАТОР (ЗАДАЁТСЯ ВРУЧНУЮ)
// ============================================================
// 🔧 ИЗМЕНИТЕ ЗДЕСЬ НАЗВАНИЕ ЕЖЕНЕДЕЛЬНОГО ВАРИАТОРА
// Доступные варианты: 'Таймер бомбы', 'Вечная мерзлота', 'Глубокий ожог', 'Кровотечение'
// ============================================================
var WEEKLY_VARIATOR_NAME = 'Таймер бомбы'; // <-- ИЗМЕНИТЕ ЗДЕСЬ НА НУЖНЫЙ ВАРИАТОР
...
let escState = {
    ...
    usedSmallTrials: [],
    nostophobiaCount: 0,
    // Еженедельный вариатор
    weeklyVariator: null
};

// ============================================================
// ПОЛУЧЕНИЕ ЕЖЕНЕДЕЛЬНОГО ВАРИАТОРА ПО ИМЕНИ
// ============================================================

function getWeeklyVariator() {
    if (typeof allVariatorsData === 'undefined') return null;
    return allVariatorsData.find(function(v) { return v.name === WEEKLY_VARIATOR_NAME; }) || null;
}
...
function isWeeklyVariator(variator) {
    ...
    if (!escState.weeklyVariator) return false;
    return variator.name === escState.weeklyVariator.name;
}
...
function isVariatorCompatible(variator, selectedVariators, trialName, playerCount, level) {
    ...
    // ============================================================
    // ДАЛЕЕ СЛЕДУЮТ ОСТАЛЬНЫЕ ПРАВИЛА
    // ============================================================
...
// ============================================================
// ПОЛУЧЕНИЕ ВАРИАТОРОВ ДЛЯ УРОВНЯ (С УЧЁТОМ ЕЖЕНЕДЕЛЬНОГО)
// ============================================================
...
function getVariatorsForLevel(level, mapName, playerCount) {
    ...
    var weeklyVar = getWeeklyVariator();
    if (weeklyVar) {
        escState.weeklyVariator = weeklyVar;
        console.log('📌 Еженедельный вариатор:', weeklyVar.name);
    } else {
        console.warn('⚠️ Еженедельный вариатор "' + WEEKLY_VARIATOR_NAME + '" не найден!');
    }
    ...
    // Доступные вариаторы (исключаем запрещённые)
    ...
    if (level >= 21) {
        ...
        // 1. Добавляем еженедельный вариатор (всегда первый)
        if (weeklyVar && weeklyVar.name) {
            result.push(weeklyVar);
            usedNames.push(weeklyVar.name);
            console.log('✅ Еженедельный (обязательный):', weeklyVar.name);
        }
        
        // 2. Добавляем психохирургию и двойные задания
        if (psycho && usedNames.indexOf(psycho.name) === -1) {
            result.push(psycho);
            usedNames.push(psycho.name);
            console.log('✅ Психохирургия (обязательная)');
        }
        if (doubleTasks && usedNames.indexOf(doubleTasks.name) === -1) {
            result.push(doubleTasks);
            usedNames.push(doubleTasks.name);
            console.log('✅ Двойные задания (обязательные)');
        }
        
        // 3. Добавляем остальные вариаторы до 8
        var others = availableVariators.filter(function(v) {
            return usedNames.indexOf(v.name) === -1;
        });
        
        var shuffled = others.slice().sort(function() { return Math.random() - 0.5; });
        
        for (var i = 0; i < shuffled.length && result.length < MAX_VARIATORS; i++) {
        ...
        // Если всё ещё не хватает до 8, добираем принудительно
        if (result.length < MAX_VARIATORS) {
            var remaining = others.filter(function(v) { return usedNames.indexOf(v.name) === -1; });
            for (var j = 0; j < remaining.length && result.length < MAX_VARIATORS; j++) {
                result.push(remaining[j]);
                usedNames.push(remaining[j].name);
                console.log('✅ Добавлен (принудительно):', remaining[j].name);
            }
        }
    ...
    // Определяем количество вариаторов для уровня
    var count;
    if (level === 1) {
        count = 2;
    ...
    console.log('🎯 Нужно вариаторов (включая еженедельный):', count);
    ...
    // 1. Добавляем еженедельный вариатор (всегда первый, если есть)
    if (weeklyVar && weeklyVar.name) {
        // Проверяем, что еженедельный вариатор совместим с остальными
        // (он будет добавлен первым, поэтому проверка на совместимость с другими будет позже)
        result.push(weeklyVar);
        usedNames.push(weeklyVar.name);
        console.log('✅ Еженедельный (обязательный):', weeklyVar.name);
    }
    
    // 2. Добавляем остальные вариаторы
    for (var k = 0; k < shuffled.length && result.length < count; k++) {
        var candidate = shuffled[k];
        // Пропускаем еженедельный вариатор, если он попался в shuffled
        if (weeklyVar && candidate.name === weeklyVar.name) continue;
        if (isVariatorCompatible(candidate, result, trialName, playerCount, level)) {
            result.push(candidate);
            usedNames.push(candidate.name);
            console.log('✅ Добавлен:', candidate.name);
    ...
    // Если не хватает вариаторов, добираем принудительно
    if (result.length < count) {
        for (var m = 0; m < shuffled.length && result.length < count; m++) {
            var candidate2 = shuffled[m];
            if (weeklyVar && candidate2.name === weeklyVar.name) continue;
            if (result.indexOf(candidate2) === -1) {
                result.push(candidate2);
                usedNames.push(candidate2.name);
...
function updateWeeklyVariatorDisplay() {
    ...
    if (nameEl) {
        if (escState.weeklyVariator) {
            nameEl.innerHTML = '<i class="fas fa-star"></i> ' + escState.weeklyVariator.name;
            console.log('✅ Отображение еженедельного вариатора обновлено:', escState.weeklyVariator.name);
        } else {
            // Пытаемся получить вариатор
            var weekly = getWeeklyVariator();
            if (weekly) {
                escState.weeklyVariator = weekly;
                nameEl.innerHTML = '<i class="fas fa-star"></i> ' + weekly.name;
            } else {
                nameEl.innerHTML = '<i class="fas fa-exclamation-circle"></i> Не найден';
            }
        }
    }
}
...
function initEscalation() {
    ...
    // Получаем и отображаем еженедельный вариатор
    var weekly = getWeeklyVariator();
    if (weekly) {
        escState.weeklyVariator = weekly;
        console.log('📌 Еженедельный вариатор установлен:', weekly.name);
    } else {
        console.warn('⚠️ Еженедельный вариатор "' + WEEKLY_VARIATOR_NAME + '" не найден в данных!');
    }
    updateWeeklyVariatorDisplay();
...
// ============================================================
// ОТРИСОВКА ШАГОВ (renderEscPlayerNames, renderEscEquipment, renderEscAmps)
// ============================================================
...
function renderEscEquipment() {
    ...
    header.innerHTML = `
        <div style="font-size: 0.8rem; color: #888; letter-spacing: 2px; text-transform: uppercase;">Шаг 3</div>
        <div style="font-size: 1.5rem; font-weight: 700; color: #ffbc9a; margin-top: 2px;">Выбор снаряжения</div>
        <div style="font-size: 0.85rem; color: #888; margin-top: 4px;">Каждому игроку предлагается 3 случайных снаряжения на выбор (нужно выбрать 1)</div>
    `;
...
function renderEscAmps() {
    ...
    header.style.cssText = 'text-align: center; margin-bottom: 20px;';
    header.innerHTML = `
        <div style="font-size: 0.8rem; color: #888; letter-spacing: 2px; text-transform: uppercase;">Шаг 4</div>
        <div style="font-size: 1.5rem; font-weight: 700; color: #ffbc9a; margin-top: 2px;">Выбор АМФ</div>
        <div style="font-size: 0.85rem; color: #888; margin-top: 4px;">Каждому игроку предлагается 3 случайных амфы на выбор (нужно выбрать 1)</div>
    `;
    container.appendChild(header);
...
function showPreviewModal(trialName, mapName, variators, level) {
    ...
    style.textContent = `
        ...
        .preview-variator-item.weekly {
            border: 2px solid #f1c40f;
            border-radius: 14px;
            padding: 4px;
            background: rgba(241, 196, 15, 0.1);
        }
        .preview-variator-item.weekly .preview-variator-name {
            color: #f1c40f;
        }
    ...
    var weeklyVarName = escState.weeklyVariator ? escState.weeklyVariator.name : null;

    var variatorsHtml = variators.map(function(v, index) {
        var nameUpper = v.name.toUpperCase();
        var delay = index * 0.05;
        var isWeekly = weeklyVarName && v.name === weeklyVarName;
        ...
        var weeklyClass = isWeekly ? 'preview-variator-item weekly' : 'preview-variator-item';
        var weeklyBadge = isWeekly ? '<div style="font-size:0.45rem; color:#f1c40f; font-weight:700; letter-spacing:0.5px; margin-top:2px;"><i class="fas fa-star"></i> НЕДЕЛЯ</div>' : '';
        
        return `
            <div class="${weeklyClass}" style="animation-delay: ${delay}s; max-width: ${maxWidth};">
...
function prepareFullResult(mapName, mapImage, trial, difficulty) {
    ...
    if (resultMap) {
        ...
        var weeklyVarName = escState.weeklyVariator ? escState.weeklyVariator.name : 'Нет';
        ...
        resultMap.innerHTML = `
            <div class="result-map-row">
                ...
                <div class="result-map-info" style="flex:1; min-width:200px;">
                    ...
                    ${escState.weeklyVariator ? `<div style="margin-top:0.5rem; padding:4px 12px; background:rgba(241,196,15,0.1); border:1px solid #f1c40f; border-radius:8px; display:inline-block;"><span style="color:#f1c40f; font-weight:700; font-size:0.8rem;"><i class="fas fa-star"></i> Еженедельный вариатор: ${weeklyVarName}</span></div>` : ''}
    ...
    if (resultVariators) {
        if (escState.variators.length === 0) {
            ...
            var weeklyVarName2 = escState.weeklyVariator ? escState.weeklyVariator.name : null;
            ...
            resultVariators.innerHTML = escState.variators.map(function(v) {
                ...
                var isWeekly = weeklyVarName2 && v.name === weeklyVarName2;
                var borderColor = isWeekly ? '#f1c40f' : 'rgba(220,90,50,0.15)';
                var bgColor = isWeekly ? 'rgba(241,196,15,0.1)' : 'rgba(0,0,0,0.3)';
                var weeklyBadge = isWeekly ? '<div style="font-size:0.5rem; color:#f1c40f; font-weight:700; letter-spacing:0.5px; margin-top:2px;"><i class="fas fa-star"></i> НЕДЕЛЯ</div>' : '';
                
                return `
                    <div class="var-item" style="display:flex; flex-direction:column; align-items:center; gap:0.4rem; max-width:100px; border:2px solid ${borderColor}; border-radius:14px; padding:6px; background:${bgColor};">
...
function resetEscalation() {
    escState = {
        ...
        usedSmallTrials: [],
        nostophobiaCount: 0,
        weeklyVariator: escState.weeklyVariator // Сохраняем еженедельный вариатор
    };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('escalationWrapper')) {
        ...
        setTimeout(function() {
            ...
            console.log('  - Еженедельный вариатор:', WEEKLY_VARIATOR_NAME);
